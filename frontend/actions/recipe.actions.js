...
export async function getOrGenerateRecipe(formData) {
  try {
    ...
    console.log("🤖 Recipe not found, generating with ChatGPT...");
...
`;
    ...
    if (!createRecipeResponse.ok) {
      ...
      const error = await createRecipeResponse.json();
      console.log("Strapi Error:", JSON.stringify(error, null, 2));

      throw new Error(error.error?.message || "Failed to save recipe");
    }
